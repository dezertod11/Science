...
k = 3
def U(fi):
    return 4*k*(1 - fi**2)**(2*k - 2)*((4*k - 1)*fi**2 - 1)
def x(fi):
    return 3*fi/(8*(1 - fi**2))+fi/(4*(1 - fi**2)**2) + 3/8*math.atanh(fi)

fi_min = 0
U_min = U(fi_min)
W = U_min
# W = -0.1
fi_max =  math.sqrt(3/11)
U_max = U(fi_max)
...
plt.plot(np.arange(x0, xn, h), U_array, label="k = 3", color='blue', linewidth=2)#график U(x)
